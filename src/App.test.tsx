...
describe("App", () => {
    ...
    test("Renders the data after it is loaded", async () => {
    ...
    test("Removes item", async () => {
        let data = await screen.findAllByRole("listitem");

        const thirdElement = data[2];

        const removeButton = within(thirdElement).getByText("Remove");
        fireEvent.click(removeButton);

        data = await screen.findAllByRole("listitem");

        expect(data).toHaveLength(testData.length - 1);
        expect(screen.queryByText(testData[2].email)).not.toBeInTheDocument();
    });

    test("Filters data", async () => {
        const filterInput = await screen.findByRole("textbox");
        fireEvent.change(filterInput, { target: { value: testData[0].email } });
    ...
    test("Reset button works", async () => {
        ...
        fireEvent.change(input, { target: { value: testData[0].email } });
